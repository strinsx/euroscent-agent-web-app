    ...
    <div className="flex items-end gap-0.5 h-10">
      {values.map((v, i) => (
        <motion.div
          ...
          style={{
            background: color,
            opacity: i === values.length - 1 ? 1 : 0.2 + (i / values.length) * 0.6,
          }}
          initial={{ height: 0 }}
    ...
    async function fetchData() {
      try {
        ...
        const [productsRes, ordersRes] = await Promise.all([
          fetch("https://euroscent-agent-web-app-server.onrender.com/api/products", { headers }),
          fetch("https://euroscent-agent-web-app-server.onrender.com/api/get-all-orders", { headers }),
        ]);
  ...
  const topProducts = useMemo(() => {
    ...
    return products
      .map((product) => {
        ...
        return { ...product, sold: counter[id]?.sold || 0 };
      })
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <p className="text-sm text-[#1f1f1f]/40">Loading dashboard...</p>
      </div>
    );
  }
      ...
      <header className="fixed top-0 inset-x-0 z-40 bg-[#1f1f1f] h-14 flex items-center justify-between px-6 md:px-12">
        <div className="flex items-center gap-6">
          ...
          >
            ...
            <span className="text-[10px] tracking-[0.25em] uppercase">Back to Shop</span>
          </Link>
      ...
      <div className="pt-14 px-6 md:px-12">
        ...
        <motion.div className="py-12" {...fadeUp(0)}>
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight">
            Admin{" "}
            <span className="text-[#1f1f1f]/20 italic">Dashboard</span>
          </h1>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
          <motion.div {...fadeUp(0.2)} className="border border-[#1f1f1f]/[0.08] p-6">
            ...
            <SparkBar values={monthlyRevenue} />
          </motion.div>
          ...
          <motion.div {...fadeUp(0.25)} className="border border-[#1f1f1f]/[0.08] p-6">
            ...
            <SparkBar values={monthlyOrders} color="#4b6bfb" />
          </motion.div>
        </div>

        {/* TOP PRODUCTS + LOW STOCK */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          <motion.div {...fadeUp(0.3)} className="border border-[#1f1f1f]/[0.08] p-6">
            <p className="text-[9px] tracking-[0.28em] uppercase text-[#1f1f1f]/35 mb-5">
              Top Products
            </p>
            <div className="space-y-4">
              {topProducts.map((product, i) => (
                <div key={product._id || product.id} className="flex items-center gap-3">
                  <span className="text-[10px] text-[#1f1f1f]/20 w-4">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold truncate">{product.name}</p>
                    <p className="text-[9px] text-[#1f1f1f]/30">{product.sold} sold</p>
                  </div>
              ...
              {lowStockProducts.map((p) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-[9px] text-[#1f1f1f]/35">{p.house}</p>
                  </div>
      ...
      <AnimatePresence>
        {selectedOrder && (
          <>
            ...
            <motion.div
              className="fixed inset-x-4 md:inset-x-auto md:left-1/2 md:-translate-x-1/2 top-1/2 -translate-y-1/2 bg-white z-50 w-full md:w-[440px] p-8 shadow-xl"
            ...
            >
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  <p className="text-[9px] tracking-[0.3em] uppercase text-[#1f1f1f]/35 mb-1">
                    Order Details
                  </p>
                  <h2 className="text-lg font-bold tracking-tight">{selectedOrder.name}</h2>
                </div>
                <button
                  onClick={() => setSelectedOrder(null)}
              ...
              <div className="space-y-1 mb-6">
                {[
                  ...
                  <div
                    key={label}
                    className="flex flex-col gap-1 py-3 border-b border-[#1f1f1f]/[0.05]"
                  >
